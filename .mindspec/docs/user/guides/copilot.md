...
- `.github/prompts/*.prompt.md` — workflow prompt commands (`/ms-spec-init`, `/ms-spec-approve`, etc.)
...
**Prompt commands**: Use `/ms-spec-init`, `/ms-spec-approve`, etc. in the chat panel — these are defined in `.github/prompts/` and instruct Copilot to run the corresponding `mindspec` CLI command.
