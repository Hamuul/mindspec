...
### Plan Artifact
When Plan Mode starts, create `docs/specs/<id>/plan.md` using the template at `docs/templates/plan.md`. Edit iteratively during planning — it is always readable on disk. On approval, update frontmatter to `status: Approved` and record `approved_at`, `approved_by`, `bead_ids`, `adr_citations`. Frontmatter is the single source of truth for plan status.
